...
func Command() *cobra.Command {
	...
	result := &cobra.Command{
		Use:   "rus-bel-import",
		Short: "Imports rus-bel dictionary",
		Long:  "Imports rus-bel dictionary",
...
func createIndex() error {
	...
	err := storage.Put("/dict-rus-bel", map[string]interface{}{
...
func processArticle(s []byte) parsedArticle {
	...
	title := sms[0][1]
	slug := textutil.Slugify(textutil.RomanizeRussian(title))
...
func indexArticles(as []parsedArticle) error {
	...
	for i, a := range as {
		...
		if err := storage.Put("/dict-rus-bel/_doc/"+id, map[string]interface{}{
			"Title":    a.title,
			"Headword": a.hws,
			"Suggest":  a.suggests,
			"Prefix":   a.prefixes,
