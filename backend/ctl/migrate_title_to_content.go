...
func MigrateTitleToContentCommand() *cobra.Command {
	c := &migrateTitleToContentController{}
	result := &cobra.Command{
		Use: "migrate-title-to-content",
		Run: c.Run,
	}
...
type migrateTitleToContentController struct {
	sourceIndexID string
	targetIndexID string
}

func (c *migrateTitleToContentController) Run(cmd *cobra.Command, args []string) {
	if err := c.run(); err != nil {
		log.Fatal(err)
	}
}

func (c *migrateTitleToContentController) run() error {
	...
	err = storage.Scroll("dict-"+c.sourceIndexID, nil, func(rawhits []json.RawMessage) error {
		for _, rawhit := range rawhits {
			...
			a.Source["Content"] = "<p><v-hw>" + a.Source["Title"].(string) + "</v-hw></p>\n" + a.Source["Content"].(string)
...
func (c *migrateTitleToContentController) flushBuffer(buff *bytes.Buffer) error {
