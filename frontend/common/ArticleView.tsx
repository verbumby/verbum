...
const IconExternalController: React.FC<{ a: Article }> = ({ a }) => {
    ...
    return (
        <a
            ref={el}
            href={`/${a.DictionaryID}/${a.ID}`}
            className="btn btn-link ms-2"
            target="_blank"
            data-bs-toggle="tooltip"
            data-bs-title="Адчыніць артыкул асобна"
            data-bs-delay={delayConfig}
            rel="noopener"
        >
            <IconExternal />
        </a>
...
const IconCopyLinkController: React.FC<{ a: Article }> = ({ a }) => {
    ...
    const onClick = () => {
        ...
        navigator.clipboard.writeText(
            `${protocol}//${host}/${a.DictionaryID}/${encodeURIComponent(a.ID)}`,
        )
        setActivated(true)
        window.setTimeout(() => {
            setActivated(false)
    ...
    return (
        <button
            ...
            onClick={onClick}
            data-bs-toggle="tooltip"
            data-bs-title="Капіраваць простую спасылку на артыкул"
            data-bs-delay={delayConfig}
        >
